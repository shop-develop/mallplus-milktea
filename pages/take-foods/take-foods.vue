<template>
	<view class="container">
		<view v-if="!order" class="d-flex w-100 h-100 flex-column just-content-center align-items-center">
						...
						<list-cell :hover="false">
							<view class="w-100 d-flex align-items-center">
								<view class="d-flex flex-column w-60">
									<view class="w-100 font-size-lg text-color-base text-truncate">{{ order.storeName }}</view>
								</view>
						...
						<list-cell :hover="false" padding="50rpx 30rpx">
							<view class="w-100 d-flex flex-column">
								<view class="d-flex align-items-center just-content-center" >
									<view class="sort-num">{{ order.id }}</view>
								</view>
								<!-- steps begin -->
								<view class="d-flex just-content-center">
									<view class="steps d-flex flex-column" :class="{'w-80': order.status == 15, 'w-100': order.status == 2}">
								...
								<view class="w-100 d-flex flex-column position-relative mt-30" style="margin-bottom: -40rpx;">
									<view class="w-100 d-flex align-items-center mb-40" v-for="(good, index) in order.orderItemList" :key="index">
										<view class="d-flex flex-column w-60 overflow-hidden">
											<view class="font-size-lg text-color-base mb-10 text-truncate">{{ good.productName }}</view>
											<view class="font-size-sm text-color-assist text-truncate" v-if="good.productAttr">{{ good.productAttr }}</view>
										</view>
										<view class="d-flex w-40 align-items-center justify-content-between pl-30">
											<view class="font-size-base text-color-base">x{{ good.productQuantity }}</view>
											<view class="font-size-base text-color-base font-weight-bold">￥{{ good.productPrice }}</view>
										</view>
									</view>
								</view>
								<!-- goods end -->
...
<script>
	...
	export default {
		...
		async onLoad() {
        			let params = { pageSize: 50, status: 0 };
