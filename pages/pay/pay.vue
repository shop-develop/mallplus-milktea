<template>
	<view class="container position-relative">
					...
					<list-cell @click="chooseAddress">
						<view class="w-100 d-flex flex-column">
							<view class="d-flex align-items-center justify-content-between mb-10">
								<view class="font-size-extra-lg text-color-base">{{ addressData.detailAddress }}</view>
								<image src="/static/images/navigator-1.png" class="arrow"></image>
							</view>
							<view class="d-flex text-color-assist font-size-sm align-items-center">
								<view class="mr-10">{{ addressData.name }}</view>
								<view class="mr-10">{{ addressData.postCode==0 ? '先生' : '女士' }}</view>
								<view>{{ addressData.phoneNumber }}</view>
							</view>
						</view>
					</list-cell>
				</template>
			...
			<view class="section-2">
				<view class="cart d-flex flex-column">
					<list-cell last v-for="(item, index) in cart" :key="index">
						<view class="w-100 d-flex flex-column">
							...
							<view class="text-truncate font-size-base text-color-assist" v-if="item.productAttr">
								{{ item.productAttr }}
							</view>
						</view>
					</list-cell>
					<template v-if="orderType == 'takeout'">
						...
						<list-cell last v-if="store.delivery_cost > 0">
							<view class="w-100 d-flex font-size-base align-items-center justify-content-between">
								<view>配送费</view>
								<view>￥{{ parseFloat(calcAmount.freightAmount) }}</view>
							</view>
						</list-cell>
					</template>
				</view>
				...
				<list-cell last>
					<view class="flex-fill d-flex justify-content-end align-items-center">
						<view>总计￥{{ calcAmount.totalAmount }},实付</view>
						<view class="font-size-extra-lg font-weight-bold">￥{{ calcAmount.payAmount }}</view>
					</view>
				</list-cell>
			</view>
			<!-- 购物车列表 end -->
		...
		<view class="w-100 pay-box position-fixed fixed-bottom d-flex align-items-center justify-content-between bg-white">
			...
			<view class="font-size-lg flex-fill">￥{{ calcAmount.payAmount }}</view>
	...
	export default {
		...
		async onLoad(opt) {
		 let option = JSON.parse(opt.dataJson);
			...
			let params = { cartIds: option.cartIds, type:2 };
            let data = await Api.apiCall('get', Api.order.preOrder, params);
			this.cart =data.cartPromotionItemList
		...
		methods: {
			...
			async pay() {
				...
				let order = this.orderType == 'takein' ? orders[0] : orders[1]
				 ...
				 let params = {};
				  params = {orderType:1, type: 2, cartIds: this.cartIds, addressId: this.addressId, couponId: this.couponId, content: this.form.remark };
