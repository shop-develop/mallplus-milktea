			...
			<view class="sign-in-info">
				<view class="left">
					<view class="d-flex align-items-baseline" @tap="flow">
						...
						<view class="number">{{ member.integration }}</view>
		...
		<view style="padding: 0 30rpx;">
			...
			<view clas="d-flex flex-column" v-for="(items, cate) in pointsMall" :key="cate">
				<view class="d-flex justify-content-between align-items-center mb-30">
					<view class="font-size-lg text-color-base">{{ items.name }}</view>
				...
				<view class="d-flex flex-wrap justify-content-between">
					<block v-for="(item, key) in items.list" :key="key">
						<view class="d-flex bg-white flex-column align-items-stretch point-box" @tap="detail(cate, item.id)">
							<image :src="item.icon ? item.icon : '/static/images/integrals/ticket.png'" class="w-100" mode="widthFix"></image>
							<view class="d-flex flex-column overflow-hidden">
								...
								<view class="d-flex align-items-center">
									<view class="d-flex align-items-baseline">
										<view class="font-size-base text-color-primary mr-10">{{ item.price }}</view>
									...
									<view  v-if="item.amount > 0"class="d-flex align-items-baseline">
										<view class="font-size-base text-color-primary mr-10">{{ parseFloat(item.price) }}</view>
										<view class="font-size-sm text-color-assist">元</view>
									</view>
								</view>
								<view class="font-size-sm text-color-assist">剩余{{ item.stock }}件</view>
							</view>
						</view>						
					</block>
				</view>
...
<script>
	...
	export default {
		...
		let params = {  };
			...
			let list = await Api.apiCall('get', Api.goods.giftTypeAndGift, {});
			this.pointsMall = list;
		},
		methods: {
